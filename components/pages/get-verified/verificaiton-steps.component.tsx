...
import { useGenerateCouponMutation } from "@/services/coupon";

const VerificationSteps = () => {
  const [verifyProfile, { data, isLoading, isSuccess, isError, error }]: any =
    useVerifyProfileMutation();
  const [
    generateCoupon,
    {
      ...
      isSuccess: generatedCouponSuccessfully,
      isError: generatedCouponError
    }
  ]: any = useGenerateCouponMutation();
  ...
  const handleSubmit = () => {
    if (
      ...
      userdata?.user?.bio !== null &&
      userdata?.user?.category !== null &&
  ...
  useEffect(() => {
    if (generatedCouponSuccessfully && couponData?.status === true) {
      setCouponCode(couponData?.coupon);
      toast({
        title: "Coupon generated"
      });
    } else if (generatedCouponSuccessfully && couponData?.status === false) {
      toast({
        title: "Oops!",
        description: `${couponData?.message}`
      });
    } else if (generatedCouponError) {
      toast({
        title: "Oops!",
        description: "Error generating coupon"
      });
    }
  }, [generatingCoupon, generatedCouponSuccessfully, generatedCouponError]);
        ...
        <div className="w-full mt-9 pb-[75px] ">
          ...
          <div className="w-full flex flex-col items-center justify-between mt-1 md:mt-9 md:flex-row ">
            <div className="w-full mt-4 md:w-2/5 md:mt-0">
              <div className="w-full">
                <p className="font-normal text-sm md:text-lg text-left md:max-lg:text-base ">
                  Step 1
                </p>
                <p className="font-normal text-xs md:text-sm mt-5 ">
                  ...
                  and correctly fill all the fields{" "}
                </p>
                <p className="font-normal text-[10px] md:text-xs mt-3 ">
                  Make sure to sell yourself using images and lots of
                  information{" "}
                </p>
              </div>
            </div>
            <div className="w-full mt-4 md:w-2/5 md:mt-0">
              <div className="w-full">
                ...
                <Label
                  htmlFor="photo-front"
                  className="flex items-start justify-start gap-4 w-full mt-5"
                >
                  <input
                    onChange={(e) => {
                      handlePhoto(e);
          ...
          <div className="w-full flex flex-col items-center justify-between mt-12 md:flex-row md:max-lg:flex-col md:max-lg:items-start ">
            <div className=" w-full md:w-2/5 md:max-lg:w-full">
              <div className="w-full">
                ...
                <p className="font-normal text-xs md:text-sm mt-5 ">
                  Pay Verification fee of NGN 2, 000
                </p>
                ...
                <div className="w-full flex items-center justify-start gap-[43px] mt-8 md:max-lg:justify-start md:max-lg:gap-8 ">
                  <Button
                    ...
                    className="w-[142px] text-xs bg-primary_color rounded-[12px] p-3"
                  >
                    {generatingCoupon ? "Generating coupon" : "Generate Coupon"}
                  </Button>
                  <Input
                ...
                <div className="w-full mt-12 ">
                  <p className="text-xs font-normal underline text-center cursor-pointer md:max-lg:text-start">
                    Fund Wallet
                  </p>
                </div>
              </div>
            ...
            <div className=" w-full flex items-center justify-center md:block md:w-2/5 mt-14 md:mt-0 md:max-lg:w-full md:max-lg:mt-8 ">
              <Button
                ...
                className="w-[248px] m-auto md:m-0 bg-primary_color rounded-[12px] py-3"
              >
    ...
    <>
      <div className="w-full flex items-center justify-between mt-[25px] ">
        <div className="w-1/2 flex items-center justify-start gap-[12px]">
          ...
          <Label htmlFor="photo-front" className="text-xs md:text-sm">
            Photo Id Front
          </Label>
        </div>
        <div className="w-1/2 flex items-center justify-start gap-[12px]">
          ...
          <Label htmlFor="photo-back" className="text-xs md:text-sm">
            Photo Id Back
          </Label>
        </div>
      </div>
