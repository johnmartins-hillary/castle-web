...
import { useCreateUsersPortfoliosMutation, useEditUsersPortfoliosMutation } from "@/services/user";
import { useEffect, useState } from "react";

const  EditPortfolioModal = ({openModal,setOpenModal,roleValue,companyValue,startDateValue,endDateValue,worksValue,id}:any) => {

  const [editUsersPortfolios,{data,isSuccess,isLoading,isError,error}]:any = useEditUsersPortfoliosMutation()
  const  [role,setRole] = useState(roleValue)
  const  [company,setCompany] = useState(companyValue)
  const  [startDate,setStartDate] = useState(startDateValue)
  const[works,setWorks] = useState(worksValue)
  const  [endDate,setEndDate] = useState(endDateValue)
  const {toast} = useToast()
    const handleSubmit=()=>{
      const data = {role:role,company:company,start:startDate,endDate:endDate,still_works_there:works,id:id}
      editUsersPortfolios(data)
