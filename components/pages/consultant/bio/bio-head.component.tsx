...
import ShareProfileModal from "../../profile/modals/share-profile- modal.component";
import { useState } from "react";
const BioHead = () => {
  const params = useParams<any>();
  const [openModal, setOpenModal] = useState(false);
  const { data, isLoading, isSuccess }: any = useGetSingleUserQuery({
    id: params?.index?.[0]
  });
  const categories = data?.category;
  const socialLinks = data?.social;
  const cons_id = data?.user?.id;
  const const_name = data?.user?.name;
  const socialMedia = [
    {
    ...
    <>
      ...
      <div className="w-full">
        ...
        <div className=" w-full mt-[20px] md:mt-0 md:w-1/2">
          <div className="w-auto flex items-center justify-start gap-1 ">
            {!isSuccess ? (
              <Skeleton className=" w-[94px] h-[13px] rounded-none " />
            ) : (
              <p className=" text-xs font-light text-left md:text-right md:text-sm ">
                Category:
              </p>
            )}
            {categories?.map(({ category_title, id }: any, index: number) => (
              ...
              >
                <p className=" text-[11px] md:text-xs ">{category_title}</p>
                {categories?.length !== index + 1 && (
                  <hr className=" w-[2px] h-[13px] bg-black" />
                )}
              </div>
            ))}
          </div>
